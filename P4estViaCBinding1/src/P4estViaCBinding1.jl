...
@reexport module libp4est
  ...
  let
    flags = [
      "-I", joinpath(dirname(dirname(@__DIR__)), "p4est_include"),
      "-I", joinpath(dirname(dirname(@__DIR__)), "mpi_include"),
      ...
      "-D", "P2EST_DATA_UNINITIALIZED=", # this does not seem to help - I needed to comment the offending line
  ...
  # Missing MPI stuff
  const c"MPI_Datatype" = MPI_Datatype
  const c"MPI_Comm" = MPI_Comm
  const c"MPI_File" = MPI_File
